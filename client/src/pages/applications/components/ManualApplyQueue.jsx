...
export default function ManualApplyQueue() {
  ...
  const queue = (items || []).filter((a) => a.manualApply === true);
  ...
  if (status === 'loading' && !queue.length) {
    return (
      <Card>
        <Card.Body className="flex items-center justify-center py-10 text-gray-400 gap-2">
  ...
  if (!queue.length) {
    return (
      <Card>
        <Card.Body className="py-10 text-center text-sm text-gray-400">
  ...
  return (
    <Card>
      <Card.Header className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-gray-900">Manual Apply Queue</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            These jobs redirect to the company's own portal — apply there, then mark done.
          </p>
        </div>
        <Badge variant="warning">{queue.filter((a) => a.status === 'pending_manual').length} pending</Badge>
      </Card.Header>
      ...
      <Card.Body className="divide-y divide-gray-100 p-0">
        {queue.map((app) => {
          ...
          return (
            <div key={app._id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50">
              {/* Status icon */}
      ...
      <Card.Footer className="text-xs text-gray-400">
